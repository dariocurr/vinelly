...
while ($num_page <= $nb_page) {
	...
	$pdf->SetXY(120, 30);
	$pdf->Cell(100, 8, $fatt_totale['nome'] . " " . $fatt_totale['cognome'], 0, 0, '');
	$pdf->SetXY(120, 35);
	$pdf->Cell(100, 8, $indirizzo['via'], 0, 0, '');
	$pdf->SetXY(120, 40);
	$pdf->Cell(100, 8, $indirizzo['cap'] . " " . $indirizzo['citta'] . ", " . $indirizzo['stato'], 0, 0, '');
	...
	if ($num_page == $nb_page) {
		...
		$pdf->Rect(5, 231, 200, 8, "D");
		$pdf->SetFont('Arial', '', 10);
		$pdf->SetXY(97, 231);
		$pdf->Cell(63, 8, $str, 0, 0, '');
	}
	...
	// scheletro tabella
	$pdf->SetLineWidth(0.1);
	$pdf->Rect(5, 55, 200, 176, "D");
	$pdf->Line(5, 63, 205, 63);
	$pdf->Line(145, 55, 145, 231);
	$pdf->Line(158, 55, 158, 231);
	$pdf->SetXY(1, 55);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(140, 8, "Prodotto", 0, 0, 'C');
	$pdf->SetXY(145, 55);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(13, 8, "Qta", 0, 0, 'C');
	$pdf->SetXY(156, 55);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(50, 8, "Prezzo", 0, 0, 'C');
	...
	while ($i < $count) {
		...
		$pdf->SetXY(7, $y + 9);
		$pdf->Cell(140, 5, $ordine['nome'], 0, 0, 'L');
		//quantità
		$pdf->SetXY(145, $y + 9);
		$pdf->Cell(8, 5, $ordine['qta'], 0, 0, 'R');
		//prezzo
		$prezzo = $ordine['prezzo'] * $ordine['qta'] . "€";
		$prezzo = iconv('UTF-8', 'windows-1252', $prezzo);
		$pdf->SetXY(187, $y + 9);
		$pdf->Cell(-12, 5, $prezzo, 0, 0, 'C');
